...
//生产者，从work到t0、t1
//高优先级生产
void sharedata_interrupt_ready( void *tWork, void *t0, void *t1, int len )
{
	((U8*)tWork)[0] = 0;	// [0] 是.m_is_busy = 0;
	((U8*)tWork)[1] = 1;   // [1] 是.m_is_hot = 1;

	if( !(((U8*)t0)[0]) ) memcpy( t0, tWork, len ) ;
	if( !(((U8*)t1)[0]) ) memcpy( t1, tWork, len ) ;
}

//消费者，从t0、t1到t2，t2是输出
//低优先级读取
void sharedata_main_followrefresh( void *t0, void *t1, void *t2, int len ) 
{
	((U8*)t0)[0] = 1;	
	if( ((U8*)t0)[1] ) memcpy( t2, t0, len ) ;
	((U8*)t0)[1] = 0;	
	((U8*)t0)[0] = 0;	
	 
	((U8*)t1)[0] = 1;	
	if( ((U8*)t1)[1] ) memcpy( t2, t1, len ) ;
	((U8*)t1)[1] = 0;	
	((U8*)t1)[0] = 0;		 
}
...
//生产者，在t0、t1间切换
//低优先级生产
void sharedata_main_ready( void *tWork, void *t0, void *t1, int len )
{
	((U8*)t0)[0] = 1;	// [0] 是.m_is_busy = 0;
	((U8*)t0)[1] = 1;	// [1] 是.m_is_hot = 1; 
	memcpy( (U8*)t0 + 2, (U8*)tWork + 2 , len - 2 );
	((U8*)t0)[0] = 0;	

	((U8*)t1)[0] = 1;	// [0] 是.m_is_busy = 0;
	((U8*)t1)[1] = 1;	// [1] 是.m_is_hot = 1; 
	memcpy( (U8*)t1 + 2, (U8*)tWork + 2 , len - 2 );
	((U8*)t1)[0] = 0;	
}

//消费者，从t0、t1到t2  .	t2是输出
//高优先级读取
void sharedata_interrupt_followrefresh( void *t0, void *t1, void *t2, int len ) 
{
	if( !(((U8*)t0)[0]) )
	if( ((U8*)t0)[1] )	// [1] 是.m_is_hot 
	{
		memcpy( t2, t0, len );
		((U8*)t0)[1] = 0;
	}
...
/*------------------------------------------------
返回 iterator-end-idx，相当于len	输入是sz，输出时末尾补00也变成了sz	
------------------------------------------------*/
#if 0
